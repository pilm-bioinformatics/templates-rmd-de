...
params:
    ...
    contrast: !r list(c("condition", "treated", "untreated"))
...
# add check function
stopifnot(file.exists(params$se_file))
stopifnot(file.exists(params$output_dir))
...
degPCA(assays(se)[["vst"]], colData(se), condition = params$metadata[1]) 
# +  ggrepel::geom_text_repel(aes(label=names)) if you wish to add sample names
```
...
comparisons = degComps(dds, contrast = params$contrast,
                       type = "ashr")
...
for (x in names(comparisons)){ 
    ...
    if (length(s) < 2){
        ...
        p=pheatmap(assays(se)[["vst"]][s,], scale = "row",
...
pattern = degPatterns(assays(se)[["vst"]][sig,],
...
resTbl = lapply(names(comparisons), function(x){
    cat("### ", x, "\n")
    p = degPlot(se, genes = significants(comparisons[[x]])[1:15],
